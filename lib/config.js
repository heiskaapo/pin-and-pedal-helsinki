...
const originsFromEnv = name => (process.env[name] || '')
  .split(/[|,\s]+/)
...
const config = {
  ...
  allowedOrigins: originsFromEnv('ALLOWED_ORIGINS'),
...
config.allowedOrigins = [...new Set([config.publicBaseUrl.toLowerCase(), ...config.allowedOrigins])];
